...
"""
    elif operation in ['tls_small', 'notls_small', 'tls_medium', 'notls_medium']:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Operation Types:
  ping         - PING latency benchmark (4 clients, 10.140.129.x)
  read         - Read-only benchmark (4 clients, 10.140.129.x)
  write        - Write-only benchmark (2 clients, 192.168.200.x)
  readwrite    - Mixed 50/50 read/write benchmark (2 clients, 192.168.200.x)
  tls_small    - Small data size variable benchmark (4 clients, 10.140.129.x)
  notls_small  - Small data size variable benchmark without TLS (4 clients, 10.140.129.x)
  tls_medium   - Medium data size variable benchmark (4 clients, 10.140.129.x)
  notls_medium - Medium data size variable benchmark without TLS (4 clients, 10.140.129.x)

Examples:
  %(prog)s -o ping -c 288 -s 64 -p 1
  %(prog)s -o tls_small -c 144 -s 1024 -p 16
  %(prog)s -o notls_medium -c 96 -s 512 -p 8
  %(prog)s -o readwrite -c 192 -s 256 -p 4
    ...
    parser.add_argument('-o', '--operation', 
                        type=str, 
                        required=True,
                        choices=['ping', 'read', 'write', 'readwrite', 'tls_small', 'notls_small', 'tls_medium', 'notls_medium'],

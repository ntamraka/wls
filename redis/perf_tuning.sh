...
for q in /sys/class/net/ens6np0/queues/rx-*; do
    QUEUE=$(basename $q)
...
for q in /sys/class/net/ens6np0/queues/tx-*; do
    QUEUE=$(basename $q)
...
IRQ_NUMS=$(grep "25:00.0" /proc/interrupts | cut -d: -f1 | tr -d ' ')
...
for irq in $IRQ_NUMS; do
    ...
    echo "  -> IRQ $irq -> CPUs $c0,$c1,$c2,$c3"
    #echo "$c0,$c1,$c2,$c3" > "/proc/irq/${irq}/smp_affinity_list"
